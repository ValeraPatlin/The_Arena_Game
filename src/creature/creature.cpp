...
void Creature::endurance_bar_stylea()
{
    endurance_bar->setStyleSheet("QProgressBar{"
                                 "border: 1px solid transparent;text-align: center;"
...
QGroupBox *Creature::progreess_box()
{
    //индикаторы   -----------------------------------------------------------------------
    hp_bar = new QProgressBar();
    mana_bar = new QProgressBar();
    endurance_bar = new QProgressBar();

    hp_bar_style();
    mana_bar_style();
    endurance_bar_stylea();

    QGroupBox* progreess_box = new QGroupBox();
    QVBoxLayout* prog_box = new QVBoxLayout();
    prog_box->addWidget(create_prog_bar(hp_bar, 0, 50));
    prog_box->addWidget(create_prog_bar(mana_bar, 0, 10));
    prog_box->addWidget(create_prog_bar(endurance_bar, 0, 20));
...
QProgressBar *Creature::create_prog_bar(QProgressBar *prog_bar, int min, int max)
{
    prog_bar->setRange(min, max);
